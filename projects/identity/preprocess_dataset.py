...
from src.features import split_train_test, select_highly_variable_genes, extract_and_scale_features, random_oversample
...
def main():
    ...
    adata_qc = run_qc_filtering(adata_filtered_types, min_genes=500, max_genes=6000, max_mt_percent=15.0, min_counts=1000)
    ...
    X_train, y_train, X_test, y_test = extract_and_scale_features(adata_train, adata_test, selected_genes=hvg_list)
    X_train_resampled, y_train_resampled = random_oversample(X_train, y_train, random_state=42)
    ...
    np.savez_compressed(train_out_path, X=X_train_resampled, y=y_train_resampled)
    np.savez_compressed(test_out_path, X=X_test, y=y_test)
