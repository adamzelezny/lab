...
def split_train_test(adata: ad.AnnData, test_size: float=0.2, random_state: int=42) -> Tuple[ad.AnnData, ad.AnnData]:
    indices = np.arange(adata.n_obs)
    train_idx, test_idx = train_test_split(indices, test_size=test_size, random_state=random_state, stratify=adata.obs['cell_type'])
    adata_train = adata[train_idx].copy()
    adata_test = adata[test_idx].copy()
    return (adata_train, adata_test)

def select_highly_variable_genes(adata_train: ad.AnnData, n_top_genes: int=3000) -> List[str]:
    adata_tmp = adata_train.copy()
    sc.pp.highly_variable_genes(adata_tmp, n_top_genes=n_top_genes, flavor='seurat', subset=False)
    hvg_mask = adata_tmp.var['highly_variable']
    hvg_list = adata_tmp.var_names[hvg_mask].tolist()
    return hvg_list

def extract_and_scale_features(adata_train: ad.AnnData, adata_test: ad.AnnData, selected_genes: List[str]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
