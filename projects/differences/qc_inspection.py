...
# Mouse mitochondrial genes usually begin with 'mt-'
adata.var["mt"] = adata.var_names.str.startswith("mt-")

# Mouse ribosomal protein genes often begin with Rps or Rpl
...
summary = (
    ...
    .agg(
        ...
        mean_pct_ribo=("pct_counts_ribo", "mean"),
...
candidate_thresholds = {
    "low_gene_cells__n_genes_lt_200": adata.obs["n_genes_by_counts"] < 200,
    "low_gene_cells__n_genes_lt_500": adata.obs["n_genes_by_counts"] < 500,
    "high_gene_cells__n_genes_gt_6000": adata.obs["n_genes_by_counts"] > 6000,
    "high_gene_cells__n_genes_gt_7000": adata.obs["n_genes_by_counts"] > 7000,
    "high_mito_cells__pct_mt_gt_10": adata.obs["pct_counts_mt"] > 10,
    "high_mito_cells__pct_mt_gt_15": adata.obs["pct_counts_mt"] > 15,
    "high_mito_cells__pct_mt_gt_20": adata.obs["pct_counts_mt"] > 20,
    "low_count_cells__counts_lt_1000": adata.obs["total_counts"] < 1000,
    "high_count_cells__counts_gt_30000": adata.obs["total_counts"] > 30000,
}
...
for name, mask in candidate_thresholds.items():
    ...
    row = {
        "criterion": name,
        "cells_failed": total_failed,
        "percent_failed": pct_failed,
    }
...
save_histogram(
    adata,
    column="total_counts",
    filename="hist_total_counts_by_condition.png",
...
save_histogram(
    ...
    filename="hist_genes_detected_by_condition.png",
    title="Genes detected per cell",
    xlabel="Number of genes detected",
)

save_histogram(
    adata,
    column="pct_counts_mt",
    filename="hist_mito_percent_by_condition.png",
...
save_histogram(
    ...
    filename="hist_ribo_percent_by_condition.png",
    title="Ribosomal percentage per cell",
    xlabel="Percent ribosomal counts",
)
...
save_scatter(
    ...
    filename="scatter_counts_vs_genes.png",
    title="Total counts vs genes detected",
    xlabel="Total counts",
...
save_scatter(
    ...
    x_col="total_counts",
    y_col="pct_counts_mt",
    filename="scatter_counts_vs_mito.png",
    title="Total counts vs mitochondrial percentage",
    xlabel="Total counts",
    ylabel="Percent mitochondrial counts",
)

save_violin(
    adata,
    columns=["total_counts", "n_genes_by_counts", "pct_counts_mt", "pct_counts_ribo"],
    filename="violin_qc_metrics_by_condition.png",
)


# ============================================================
# SAVE QC-ANNOTATED DATASET
# ============================================================
