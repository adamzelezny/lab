...
def generate_baseline_expression(cells, genes, seed=42):
    ...
    for g in range(genes):
        pre[:, g] = np.random.normal(gene_means[g], gene_stds[g], cells)
...
def simulate_activation(pre, gene_stds, genes, cells, seed=42):
    ...
    for g in upregulated:
        activation[:, g] += np.random.normal(1.5, 0.3, cells)
    for g in downregulated:
        activation[:, g] -= np.random.normal(1.2, 0.2, cells)
    for g in heterogeneous:
        activation[:, g] += np.random.normal(0, gene_stds[g] * 1.5, cells)
...
def main():
    ...
    activation, upregulated, downregulated, heterogeneous = simulate_activation(pre, gene_stds, genes, cells, seed)
