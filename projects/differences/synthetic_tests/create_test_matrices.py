...
N_CELLS = 500
N_GENES = 500
important_genes = ['Sox2', 'Gfap', 'Olig2', 'Pdgfra', 'Mki67', 'Top2a', 'Pcna', 'Vegfa', 'Hif1a', 'Egf', 'Egfr', 'Nes', 'Aqp4', 'Vim', 'Fos', 'Jun', 'Egr1', 'Arc', 'Ntrk2', 'Bdnf', 'Cxcl10', 'Ccl2', 'Tnf', 'Il6', 'Cd68', 'Aif1', 'Snap25', 'Syp', 'Map2', 'Rbfox3', 'Mog', 'Mbp', 'Plp1', 'Pecam1', 'Kdr']
...
cell_types = np.random.choice(['tumor_cell', 'astrocyte_like', 'oligodendrocyte_like', 'microglia', 'endothelial'], size=N_CELLS, p=[0.55, 0.18, 0.12, 0.1, 0.05])
metadata = pd.DataFrame({'cell_id': [f'cell_{i}' for i in range(N_CELLS)], 'mouse_id': mouse_ids, 'condition': conditions, 'cell_type': cell_types})
...
for g in range(N_GENES):
    ...
    counts[:, g] = np.random.negative_binomial(n=dispersion, p=p, size=N_CELLS)
...
for gene in ['Mki67', 'Top2a', 'Pcna']:
    ...
    counts[activation_mask, idx] = np.maximum(0, counts[activation_mask, idx] - reduction)
dropout_prob = np.exp(-counts / 4)
dropout = np.random.random(counts.shape) < dropout_prob * 0.25
counts[dropout] = 0
count_df = pd.DataFrame(counts, index=metadata['cell_id'], columns=genes)
