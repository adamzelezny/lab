...
def global_statistics(matrix1, matrix2):
    ...
    print(f'Matrix 1: Mean = {np.mean(matrix1):.4f}, Median = {np.median(matrix1):.4f}, Std = {np.std(matrix1):.4f}')
    print(f'Matrix 2: Mean = {np.mean(matrix2):.4f}, Median = {np.median(matrix2):.4f}, Std = {np.std(matrix2):.4f}')
    print()
...
def ks_test(matrix1, matrix2):
    ...
    for col in range(matrix1.shape[1]):
        ks_stat, p_val = stats.ks_2samp(matrix1[:, col], matrix2[:, col])
        if p_val < 0.05:
            significant_cols.append((col, p_val))
    if significant_cols:
        print(f'Found {len(significant_cols)} columns with significantly different distributions:')
...
def random_forest(matrix1, matrix2):
    ...
    y = np.concatenate((np.zeros(matrix1.shape[0]), np.ones(matrix2.shape[0])))
...
def main():
    matrix1, matrix2 = load_matrices('pre_activation.csv', 'activation.csv')
