...
def make_pseudobulk(adata_raw: sc.AnnData, cell_mask: np.ndarray, sample_col: str='sample') -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_counts = pd.DataFrame(pseudobulk_matrix, index=unique_samples, columns=adata_raw.var_names)
...
def run_deseq2(counts_df: pd.DataFrame, metadata_df: pd.DataFrame, out_dir: str, prefix: str) -> dict[str, pd.DataFrame]:
    ...
    dds = DeseqDataSet(counts=counts_df, metadata=metadata_df, design_factors='variant', refit_cooks=True)
    dds.deseq2()
    results = {}
    contrasts = [('H1047R', 'R88Q', 'H1047R_vs_R88Q_control'), ('C420R', 'R88Q', 'C420R_vs_R88Q_control'), ('H1047R', 'C420R', 'H1047R_vs_C420R_direct')]
...
def run_gsea(de_results: dict[str, pd.DataFrame], output_dir: str, prefix: str) -> dict[str, pd.DataFrame]:
    ...
    for label, res_df in de_results.items():
        ...
        try:
            pre_res = gp.prerank(rnk=ranking, gene_sets='GO_Biological_Process_2023', outdir=contrast_out_dir, permutation_num=100, min_size=15, max_size=500, no_plot=True)
...
def generate_summary_report(tumor_de: dict[str, pd.DataFrame], neuron_de: dict[str, pd.DataFrame], tumor_gsea: dict[str, pd.DataFrame], neuron_gsea: dict[str, pd.DataFrame], report_path: str) -> None:
