...
def plot_pca_results(adata: sc.AnnData, output_dir: str) -> None:
    ...
    df_plot = pd.DataFrame({'PC1': pca[:, 0], 'PC2': pca[:, 1], 'Cluster': adata.obs['kmeans'], 'Variant': adata.obs['variant'], 'Sample': adata.obs['sample']})
    raw_adata = adata.raw.to_adata()
    ...
    if len(df_plot) > 20000:
        ...
        df_plot_sub = df_plot
    sns.scatterplot(ax=axes[0, 0], data=df_plot_sub, x='PC1', y='PC2', hue='Cluster', palette='Set1', s=3, alpha=0.6, hue_order=sorted(df_plot['Cluster'].unique()))
    axes[0, 0].set_title('PCA - K-Means Clusters')
    sns.scatterplot(ax=axes[0, 1], data=df_plot_sub, x='PC1', y='PC2', hue='Variant', palette='Dark2', s=3, alpha=0.6)
    axes[0, 1].set_title('PCA - Variants')
    if 'Gfap' in df_plot_sub.columns:
        sc1 = axes[1, 0].scatter(df_plot_sub['PC1'], df_plot_sub['PC2'], c=df_plot_sub['Gfap'], cmap='viridis', s=2, alpha=0.6)
    ...
    if 'Map2' in df_plot_sub.columns:
        sc2 = axes[1, 1].scatter(df_plot_sub['PC1'], df_plot_sub['PC2'], c=df_plot_sub['Map2'], cmap='plasma', s=2, alpha=0.6)
