...
def extract_sample_stats(adata: sc.AnnData, sample_name: str) -> dict:
    ...
    stats = {'Sample': sample_name, 'Cells (>=500 counts)': adata.n_obs, 'Total Vocabulary Genes': adata.n_vars, 'Expressed Genes (>=1 count)': active_genes, 'Mean UMI Counts': float(obs['total_counts'].mean()), 'Median UMI Counts': float(obs['total_counts'].median()), 'Std UMI Counts': float(obs['total_counts'].std()), 'Mean Genes Detected': float(obs['n_genes_by_counts'].mean()), 'Median Genes Detected': float(obs['n_genes_by_counts'].median()), 'Std Genes Detected': float(obs['n_genes_by_counts'].std()), 'Mean Pct Mitochondrial': float(obs['pct_counts_mt'].mean()), 'Median Pct Mitochondrial': float(obs['pct_counts_mt'].median())}
    return stats

def analyze_key_genes(adata: sc.AnnData, sample_name: str, genes: list[str]) -> list[dict]:
    results = []
    for gene in genes:
        gene_stats = {'Sample': sample_name, 'Gene': gene, 'Present': False, 'Pct Expressing Cells': 0.0, 'Mean Expression (All Cells)': 0.0, 'Mean Expression (Expressing Cells)': 0.0}
...
def main():
    ...
    genes_of_interest = ['Pik3ca', 'Pten', 'Nf1', 'Trp53', 'Gpc3', 'Gfap', 'Aldh1l1', 'Map2', 'Dlg4', 'Syp']
    all_adatas = {}
    stats_list = []
    genes_list = []
